    ...
    (wpm: number, accuracy: number): ReplayRecording | null => {
      ...
      try {
        ...
        localStorage.setItem(
          "typeuz_recordings",
          JSON.stringify([replay, ...existing.slice(0, 19)])
        );
      } catch {
        // ignore
      }
...
export default function TypingReplayView({ recordings, t, onClose }: TypingReplayViewProps) {
  ...
  if (playing) {
    const accuracy = Math.round(
      (playing.text
        .split("")
        .slice(0, playState.display?.length || 0)
        .filter((ch, i) => ch === (playState.display || "")[i]).length /
        Math.max(1, playState.display?.length || 1)) *
        100
    );
          ...
          <div className="leading-relaxed tracking-wide text-center select-none">
            {playing.text.split("").map((ch, i) => {
              ...
              if (i === (playState.display?.length || 0)) {
                return (
                  <span key={i} className={cls} style={{ color: t.accent, textDecoration: "underline" }}>
                    {ch}
                  </span>
                );
              }
