...
/**
 * Supabase'dan ko'chirilgan profillarni Convex `users` jadvaliga import qiladi.
 * Faqat owner. Har bir profil `supabase:<id>` barqaror tokenIdentifier bilan
 * yoziladi — email/parol Convex'ga ko'chmaydi (auth Anonymous), faqat profil
 * ma'lumotlari admin panelda ko'rinishi uchun.
 */
export const importProfiles = mutation({
  args: {
    ...
    /** Script orqali import uchun: to'g'ri ADMIN_PASSWORD berilsa owner sifatida ruxsat beriladi */
    adminPassword: v.optional(v.string()),
  },
  handler: async (ctx, args) => {
    // Ruxsat: owner rolidagi identifikatsiya YOKI to'g'ri ADMIN_PASSWORD
    const envPassword = process.env.ADMIN_PASSWORD || "";
    const viaPassword = !!args.adminPassword && !!envPassword && args.adminPassword === envPassword;
    let adminName = "parol-orqali";
    if (!viaPassword) {
      const admin = await requireAdmin(ctx);
      if (admin.user.role !== "owner") throw new Error("Faqat owner import qila oladi");
    ...
    await logAdminAction(
      ...
      adminName,
      "import_profiles",
      `${args.profiles.length} ta`,
      `${inserted} yangi, ${updated} yangilandi, ${skipped} o'tkazib yuborildi`
    );
